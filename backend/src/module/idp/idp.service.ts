...
import {catchError, firstValueFrom, lastValueFrom, ObservableInput} from "rxjs";
...
export class IdpService {
    ...
    async getIdpUserByUsername(realmName: string, username: string, tokenData?: string): Promise<any> {
        try {
    ...
    async getIdpUserBySub(realmName: string, sub: string,tokenData?:string): Promise<any> {
        try {
            if(!tokenData) tokenData=await this.getIdpToken(realmName);
            let url: string = KEYCLOAK_USER_URL.replace("{realm_name}", realmName);
            url = url + `/${sub}`
    ...
    async createIdpUser(idpCreatedReqData) {
        ...
        try {
            ...
            let idpUser = null;
            const {username, firstName, lastName, address, email, password} = idpCreatedReqData
            ...
            const payload = {
                ...
                groups: [KEYCLOAK_DEFAULT_USER_ROLE]
            }
            let url: string = KEYCLOAK_USER_URL.replace("{realm_name}", KEYCLOAK_REALM);
            ...
            const observable = this.httpService
                ...
                .pipe(
                    ...
                    catchError((error: any) => {
                        ...
                        let errorMessage = error.response.data.errorMessage ? error.response.data.errorMessage : error.response.data.error

                        throw new HttpException(new Error(errorMessage + " in IDP"), error.response.status);
                    }),
                );

            const idpResponse = await lastValueFrom(observable);
            if (idpResponse.status == HttpStatus.CREATED) {
                idpUser = await this.getIdpUserByUsername(KEYCLOAK_REALM, idpCreatedReqData.username, adminToken);
            }
    ...
    async getIdpGroups(tokenData?: string) {
        ...
        try {
            ...
            let url: string = KEYCLOAK_GET_ALL_GROUPS.replace("{realm_name}", KEYCLOAK_REALM);
    ...
    async getIdpGroupRoles(groupId: string, tokenData?: string) {
        ...
        try {
            ...
            const data = response.pipe(
                ...
                catchError((error: any) => {
                    console.log(error)
                    let errorMessage = error.data.errorMessage ? error.data.errorMessage : error.data.error
                    return this.throwError(error);
                }),
            );
