...
export class JwtMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    const decoded = verify(token, KEYCLOAK_PUBLIC_KEY, {
      algorithms: ['RS256'],
    });
    const sub: any = decoded?.sub;

    const { username, firstName, lastName, email, id } =
    ...
    const userGroup = allGroups.filter(
      (group) => group.name == KEYCLOAK_DEFAULT_USER_GROUP,
    );
